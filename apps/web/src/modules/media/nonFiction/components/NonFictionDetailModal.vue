...
const emit = defineEmits<{
  (e: 'close'): void
  (e: 'update', updatedNonFiction: NonFiction): void
...
function syncForm(data: NonFiction) {
  ...
  // Sum of totals or just take first? For now take first volume's total if exists
  form.volumes = data.volumes && data.volumes.length > 0 ? data.volumes[0].total : 0

  // Derived release status from first volume?
  // Use a safe check/temporary variable
  const firstVol = data.volumes && data.volumes.length > 0 ? data.volumes[0] : null
  form.releaseStatus = firstVol && firstVol.isCompleted ? 'Completed' : 'Ongoing'
...
function cancelEdit() {
  if (props.nonFiction) syncForm(props.nonFiction)
  isEditing.value = false
}
...
async function toggleStatus() {
  ...
  try {
    ...
    const updated = await NonFictionService.update(props.nonFiction.id, payload)

    toast({
      title: 'Status Updated',
      description: `Non-Fiction marked as ${newStatus}`,
    })
...
async function handleSave() {
  ...
  try {
    const payload: Partial<NonFiction> = {
      ...
      // Assume handling first volume or creating single volume entry representing total
      volumes: [
        {
          standalone: form.type === 'Standalone', // Using form.type for logic even if not in payload
    ...
    let updateData: Partial<NonFiction> | FormData = payload
    ...
    const updated = await NonFictionService.update(props.nonFiction.id, updateData)
...
async function handleDelete() {
  ...
  if (!confirm('Are you sure you want to delete this Non-Fiction? This action cannot be undone.')) {
    return
  }

  isDeleting.value = true
  try {
    await NonFictionService.delete(props.nonFiction.id)
    toast({
      title: 'Deleted',
      description: 'Removed from library',
    })
    emit('delete', props.nonFiction.id)
  } catch (err) {
    ...
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div class="relative h-48 sm:h-64 shrink-0 overflow-hidden bg-muted">
          <img
            v-if="previewUrl || nonFiction?.imageUrl"
            :src="previewUrl || nonFiction?.imageUrl"
          ...
          <div class="absolute bottom-4 left-6 right-6 text-white">
            <h2 class="text-2xl sm:text-3xl font-bold line-clamp-2">{{ nonFiction?.title }}</h2>
        ...
        <div class="flex-1 overflow-y-auto p-6">
          ...
          <div v-if="!isEditing" class="space-y-6">
            ...
            <div v-if="form.author" class="grid grid-cols-1 gap-4">
              <div class="p-3 bg-secondary/30 rounded-lg">
                <div class="text-xs text-muted-foreground uppercase tracking-wider mb-1">
                  Author
                </div>
