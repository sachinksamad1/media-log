...
<template>
  ...
  <!-- Sidebar -->
  <aside
    class="fixed top-16 left-0 h-[calc(100vh-4rem)] bg-sidebar border-t-0 border border-border z-40
           transition-all duration-300 ease-in-out flex flex-col w-50
           lg:translate-x-0"
    :class="[
      open ? 'translate-x-0' : '-translate-x-full',
      collapsed ? 'lg:w-16' : 'lg:w-50'
    ]"
  >
      ...
      <div class="space-y-1">
        <RouterLink
          ...
          :class="isActive(item.href)
            ? 'bg-primary/10 text-secondary' : 'text-sidebar-foreground hover:text-sidebar-accent-foreground hover:bg-sidebar-accent'"
        >
