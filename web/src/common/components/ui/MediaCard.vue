...
<template>
  <div class="relative group cursor-pointer transition-transform hover:-translate-y-1">
    ...
    <div class="relative aspect-[2/3] rounded-xl overflow-hidden shadow-sm hover:shadow-md bg-secondary/20">
      <LazyImage 
        :src="item.coverUrl" 
        :alt="item.title"
      ...
      <!-- Rating Badge -->
      <div v-if="hasRating" class="absolute top-2 right-2 flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-black/60 backdrop-blur-md text-yellow-400 text-xs font-bold shadow-sm">
