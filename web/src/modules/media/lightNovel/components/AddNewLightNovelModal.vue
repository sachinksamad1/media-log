...
async function handleSave() {
  ...
  try {
    ...
    let createData: Partial<LightNovel> | FormData = payload
    ...
    const created = await LightNovelService.create(createData)
    toast({
      title: 'Success',
      description: 'Light Novel added to your library',
    })
    ...
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div class="flex items-center justify-between p-6 border-b border-border bg-secondary/10">
          <h2 class="text-xl font-bold">Add New Light Novel</h2>
          <button
        ...
        <div class="p-4 border-t border-border bg-secondary/10 flex justify-end gap-3">
          ...
          >
            ...
            {{ saving ? 'Creating...' : 'Add Light Novel' }}
          </button>
        </div>
      </div>
    </div>
