...
const emit = defineEmits<{
  (e: 'close'): void
  (e: 'update', updatedLightNovel: LightNovel): void
...
function syncForm(data: LightNovel) {
...
function cancelEdit() {
  if (props.lightNovel) syncForm(props.lightNovel)
  isEditing.value = false
}
...
async function toggleStatus() {
  ...
  try {
    ...
    const updated = await LightNovelService.update(props.lightNovel.id, payload)
    
    toast({
      title: 'Status Updated',
      description: `Light Novel marked as ${newStatus}`,
    })
...
async function handleSave() {
  ...
  saving.value = true
  try {
    const payload: Partial<LightNovel> = {
      title: form.title,
      author: form.author,
    ...
    let updateData: Partial<LightNovel> | FormData = payload
    ...
    const updated = await LightNovelService.update(props.lightNovel.id, updateData)
...
async function handleDelete() {
  ...
  if (!confirm('Are you sure you want to delete this Light Novel? This action cannot be undone.')) {
    return
  }
  
  isDeleting.value = true
  try {
    await LightNovelService.delete(props.lightNovel.id)
    toast({
      title: 'Deleted',
      description: 'Removed from library',
    })
    emit('delete', props.lightNovel.id)
  } catch (err) {
    ...
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-card text-card-foreground w-full max-w-2xl rounded-xl shadow-2xl border border-border flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div class="relative h-48 sm:h-64 shrink-0 overflow-hidden bg-muted">
           <img 
             v-if="previewUrl || lightNovel?.imageUrl" 
             :src="previewUrl || lightNovel?.imageUrl" 
           ...
           <div class="absolute bottom-4 left-6 right-6 text-white">
             <h2 class="text-2xl sm:text-3xl font-bold line-clamp-2">{{ lightNovel?.title }}</h2>
        ...
        <div class="flex-1 overflow-y-auto p-6">
          ...
          <div v-if="!isEditing" class="space-y-6">
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
               ...
               <div class="p-3 bg-secondary/30 rounded-lg">
                 <div class="text-xs text-muted-foreground uppercase tracking-wider mb-1">Volumes</div>
