...
const statusClass = computed(() => {
  const status = props.movie.userStats.status
  switch (status) {
    case 'Completed': return 'bg-green-500 text-white'
    case 'Plan to Watch': return 'bg-blue-500 text-white'
    case 'Watching': return 'bg-amber-500 text-white'
  ...
  >
    ...
    <div class="relative w-full h-[260px] overflow-hidden">
      ...
      <div class="absolute top-2 right-2 px-2 py-1 rounded text-xs font-bold shadow-sm"
        :class="statusClass"
      >
        {{ movie.userStats.status }}
      </div>
    </div>

    <!-- Content -->
    <div class="p-4">
      <h4 class="font-semibold text-base line-clamp-1 mb-1" :title="movie.title">{{ movie.title }}</h4>
      <div class="flex items-center justify-between text-sm text-muted-foreground">
        <span>{{ durationText }}</span>
        <div class="flex items-center gap-1 text-accent">
          <span>⭐</span>
          <span>{{ movie.userStats.score }}</span>
        </div>
      </div>
