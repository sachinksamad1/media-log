...
export const TvSeriesService = {
  async getAll(limit?: number, cursor?: string, status?: string): Promise<TvSeriesResponse> {
    const params = new URLSearchParams()
    if (limit) params.append('limit', limit.toString())
    if (cursor) params.append('cursor', cursor)
    if (status) params.append('status', status)

    const { data } = await http.get<TvSeriesResponseDto>(`/tv-series?${params.toString()}`)
  ...
  async search(query: string): Promise<TvSeries[]> {
    ...
    const { data } = await http.get<{ data: TvSeriesDto[] }>(`/search?${params.toString()}`)
